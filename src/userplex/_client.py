...
class Userplex(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Userplex client instance.

        This automatically infers the `api_key` argument from the `USERPLEX_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("USERPLEX_API_KEY")
        if api_key is None:
            raise UserplexError(
                "The api_key client option must be set either by passing api_key to the client or by setting the USERPLEX_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("USERPLEX_BASE_URL")
        if base_url is None:
            base_url = f"https://api.userplex.app"
...
class AsyncUserplex(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncUserplex client instance.
