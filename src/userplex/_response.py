...
from ._exceptions import UserplexError, APIResponseValidationError
...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if (
            ...
            raise TypeError("Pydantic models must subclass our base model type, e.g. `from userplex import BaseModel`")
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from userplex import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `userplex._streaming` for reference",
        )


class StreamAlreadyConsumed(UserplexError):
    """
